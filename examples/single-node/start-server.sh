...
echo "  ../../bin/eventmesh-cli --no-auth health --server http://localhost:$HTTP_PORT"
echo ""
echo -e "${YELLOW}Publish an event:${NC}"
echo "  ../../bin/eventmesh-cli --no-auth publish --server http://localhost:$HTTP_PORT --topic orders.created --payload '{\"id\": 123}'"
echo ""
